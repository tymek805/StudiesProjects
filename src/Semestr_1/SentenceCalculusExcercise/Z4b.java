...
public class Z4b {
  ...
  private static String szukanySpojnik; // dany spojnik
  private static int totalOccurences; // liczba wystapien danego spojnika
	...
	switch (sym) {
       ...
       case L_NAWIAS     :
           ...
           if (sym == symbol.NEGACJA) {
               if (szukanySpojnik.equals("N")) totalOccurences++;
              ...
              switch (sym) {
                 case KONIUNKCJA  :
                     if (szukanySpojnik.equals("K")) totalOccurences++;
                     spojniki.add("K"); pobSymbol(); spojniki.addAll(formula()); break;
                 case ALTERNATYWA :
                     if (szukanySpojnik.equals("A")) totalOccurences++;
                     spojniki.add("A"); pobSymbol(); spojniki.addAll(formula()); break;
                 case IMPLIKACJA  :
                     if (szukanySpojnik.equals("C")) totalOccurences++;
                     spojniki.add("C"); pobSymbol(); spojniki.addAll(formula()); break;
                 case ROWNOWAZNOSC:
                     if (szukanySpojnik.equals("E")) totalOccurences++;
                     spojniki.add("E"); pobSymbol(); spojniki.addAll(formula()); break;
                 default: blad(3);
			  };
           };
  ...
  private static Set<String> formulaAtomowa() { //---------------------------------- formula
	...
	switch (sym) {
       ...
       case FALSUM           : 	if (szukanySpojnik.equals("F")) totalOccurences++; spojniki.add("F"); pobSymbol(); break;
       case VERUM            : 	if (szukanySpojnik.equals("V")) totalOccurences++; spojniki.add("V"); pobSymbol(); break;
    }
	return spojniki;
  }
  ...
  public static void main(String[] args) {
    if (args.length == 2) {
	  ...
	  if (sym == symbol.KONIEC) blad(0);
        ...
        else System.out.println("W formule \"" + formula + "\" spojnik " + szukanySpojnik + " wystepuje tyle razy: " + totalOccurences);
      }
	} else if (args.length == 1) {
        System.out.println("Potrzeba dwoch argumentow -> formula szukanySpojnik");
    } else
	  System.out.println("Wywolanie: Z4b \"<formula>\"");
  }  // koniec main
}    // koniec klasy